...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=34318`，进入 `Epoch 2/4`，最新人工检查到 step 62486；已保存到 `compressor_e2_s62000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=25738`，进入 `Epoch 2/3`，最新人工检查到 step 38347；已保存到 `compressor_e2_s38000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | first50 已完成 50/50 个 `.pt`，远端目录约 961G；第 51-100 个 shard 已启动同步，当前约 70%（远端 83 个 `.pt`，约 1.7T），日志未见错误 |
| 策略 | 当前第 51-100 个 shard 单路同步中；不并发，不使用 `--delete` |
