...
110K B-1L resume 记录（gpu8, 2026-05-17 22:42）：

| 项目 | 数值 |
|------|------|
| tmux session | `train-110k-b1l-resume-0-6-7-e4` |
| GPU | `CUDA_VISIBLE_DEVICES=0,6,7` |
| resume checkpoint | `/nvmessd/lifanhong/video/outputs_compressor_110k_B1L_old_4gpu_shardhint/compressor_epoch1.pt` |
...
110K B-2L epoch2-3 resume 记录（gpu8, 2026-05-18 01:15）：

| 项目 | 数值 |
|------|------|
| tmux session | `train-110k-b2l-resume-e23-1-4` |
| GPU | `CUDA_VISIBLE_DEVICES=1,2,3,4` |
| resume checkpoint | `/nvmessd/lifanhong/video/outputs_compressor_110k_B2L_old_4gpu_1_4_shardhint/compressor_epoch1.pt` |
...
| 当前状态 | 09:00 auto-stop 已触发并停止 tmux；epoch2 已完成，`avg_loss=0.885294`（`total_segs=196099`），已保存 `compressor_epoch2.pt`；停止前处于 `Epoch 3/3`，最终日志 step 60927，最新 checkpoint 为 `compressor_e3_s60000.pt`；日志未见 OOM/Traceback |

jiagpu4 数据同步记录（2026-05-18）：

| 项目 | 数值 |
|------|------|
| 当前批次 | `teacher_cache_110k_sharded_256` 前 50 个 `.pt` shard |
...
| epoch3 full eval (0-60) | 68.63%（6934/10103），日志 `/nvmessd/lifanhong/video/log_eval_compressor_110k_B1L_epoch3_jiagpu4_gpu6_0_60_full.txt` |
...
|------|-------|----------|----------|
| **Zero-shot (logit)** | — | **80.44%** (1476/1835) | **79.54%** (8036/10103) |
| B-1L (1层) | 1 | 70.08% (1286/1835) | 68.48% (6919/10103) |
| B-1L (1层) | 3 | 69.86% (1282/1835) | 68.63% (6934/10103) |
| B-2L (2层) | 1 | 69.37% (1273/1835) | 68.83% (6953/10103) |
| B-2L (2层) | 2 | 69.59% (1277/1835) | 68.86% (6958/10103) |
| B-2L (2层) | 3 | 69.21% (1270/1835) | 68.98% (6969/10103) |

Zero-shot 与 compressor 均使用 logit 方法（比较 ABCD token logit），同一口径。
Compressor 比 zero-shot 低 ~10%（0-30: 80→70, 0-60: 80→69），压缩带来的信息损失。
