...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=34318`，进入 `Epoch 2/4`，最新人工检查到 step 59884；已保存到 `compressor_e2_s59000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=25738`，进入 `Epoch 2/3`，最新人工检查到 step 36092；已保存到 `compressor_e2_s36000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | first50 已完成 50/50 个 `.pt`，远端目录约 961G；日志未见错误 |
| 策略 | 当前处于 1 小时休息窗口，已安排随后单路同步第 51-100 个 shard；不并发，不使用 `--delete` |
