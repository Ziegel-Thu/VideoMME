...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=34318`，进入 `Epoch 2/4`，最新人工检查到 step 51605；已保存到 `compressor_e2_s51000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | 已恢复到 `start_epoch=1, global_step=25738`，进入 `Epoch 2/3`，最新人工检查到 step 29212；已保存到 `compressor_e2_s29000.pt`；日志未见 OOM/Traceback |
...
| 当前状态 | 已完成 32/50 个 `.pt`，远端目录约 648G；rsync 进度约 66%，日志未见错误 |
| 策略 | first50 完成后休息 1 小时，再单路同步第 51-100 个 shard；不并发，不使用 `--delete` |
