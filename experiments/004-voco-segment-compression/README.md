...
**注意：epoch1-3 评测的 200 条 test 来自 `0_30` 视频子集；epoch4-5 评测的 200 条 test 来自完整 `0-60` test set（含 `30_60` 视频）。覆盖视频不同导致直接对比需要谨慎。**

| 配置 | epoch1 | epoch2 | epoch3 | epoch4 | epoch5 |
|------|--------|--------|--------|--------|--------|
| B-1L | 66.5% | 71.0% | 71.0% | 59.0% | 59.5% |
| B-2L | - | - | 69.0% | - | - |
| D-1L | - | - | 63.0% | 61.5% | 61.5% |
| BD-1L | - | - | 69.5% | - | - |

epoch4-5 的 B-1L 准确率从 71% 降到 59%，可能原因：
1. 评测用的 200 条 test 样本变了（新增了 30_60 视频，覆盖率不同）
2. 训练数据仍是 10K（仅 0_30 子集），但评测混入了 30_60 视频，compressor 没见过更长视频
3. 需要在**相同的 200 条 test 子集**上重新评测确认是否真的退化
